...
export default function BottomNavigation({ notificationCount = 0 }: BottomNavigationProps) {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-700 z-50 safe-area-mobile">
      <div className="flex items-center justify-around h-16 sm:h-18 px-2 sm:px-4 max-w-md mx-auto">
        {navItems.map((item) => {
          ...
          return (
            <Link key={item.href} href={item.href}>
              <div
                className={cn(
                  "flex flex-col items-center justify-center p-2 sm:p-3 rounded-lg transition-colors relative btn-touch min-w-0",
                  item.isActive
              ...
              >
                ...
                <span className="text-[10px] sm:text-xs mt-1 font-medium truncate max-w-full">{item.label}</span>
              </div>
            </Link>
          );
        })}
